...
class VoyagerUserController extends BaseVoyagerController
{
        ...
        foreach ($rows as $key => $row) {
            if ($row->field === $request->input('type')) {
                ...
                foreach ($relationshipOptions as $relationshipOption) {
                    ...
                    if ($request->type == 'user_belongsto_role_relationship' && Auth::user()->id != 1)
                    {
                        if ($relationshipOption->{$options->key} == 1) continue;
                    }
